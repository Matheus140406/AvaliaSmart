...
import { generateExam } from "@/services/ai/exam-generator.service";

/**
 * POST /api/ai/exam-generator — Etapa 1 da expansão de produtividade docente.
 * Recebe um documento (texto colado OU foto/scan) e gera 5 questões de
 * múltipla escolha + 2 discursivas com gabarito e critérios de correção.
 *
 * multipart/form-data com EXATAMENTE um dos dois: `text` (string colada) ou
 * `image` (foto/scan do documento) — mesmo padrão de "ou/ou" já usado no
 * resto do produto (ex: export/pdf/ai-summary usa classId XOR studentId).
 */
...
export const POST = withTenant(async (request, user) => {
  if (!WRITE_ROLES.has(user.role)) {
    throw forbidden("Sem permissão para gerar provas.");
  }

  const accessBlock = await requireAiFeature(user.tenantId, "examGenerator");
  ...
  if (hasImage) {
    ...
    try {
      ...
      console.error("[exam-generator] falha ao extrair texto da imagem:", err);
