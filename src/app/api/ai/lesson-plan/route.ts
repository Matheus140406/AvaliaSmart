...
import { generateLessonPlan } from "@/services/ai/lesson-plan.service";

/**
 * POST /api/ai/lesson-plan — Etapa 3 da expansão de produtividade docente.
 * Mesmo contrato de entrada das Etapas 1-2: multipart/form-data com
 * EXATAMENTE um dos dois, `text` ou `image`.
 */
...
export const POST = withTenant(async (request, user) => {
  if (!WRITE_ROLES.has(user.role)) {
    throw forbidden("Sem permissão para gerar plano de aula.");
  }

  const accessBlock = await requireAiFeature(user.tenantId, "lessonPlan");
  ...
  if (hasImage) {
    ...
    try {
      ...
      console.error("[lesson-plan] falha ao extrair texto da imagem:", err);
